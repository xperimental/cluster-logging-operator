package certificates

import (
	"fmt"
	"os/exec"

	"sigs.k8s.io/yaml"

	"github.com/go-logr/logr"
	"github.com/openshift/cluster-logging-operator/internal/utils"
)

func GenerateCertificates(logger logr.Logger, namespace, scriptsDir, logStoreName, workDir string) (err error, updated bool, output []interface{}) {
	script := fmt.Sprintf("%s/cert_generation.sh", scriptsDir)
	return runCertificatesScript(logger, namespace, logStoreName, workDir, script)
}

func runCertificatesScript(logger logr.Logger, namespace, logStoreName, workDir, script string) (err error, updated bool, output []interface{}) {
	updated = false

	logger.V(3).Info("Running script", "script", script, "workDir", workDir, "namespace", namespace, "logStoreName", logStoreName)
	cmd := exec.Command(script, workDir, namespace, logStoreName)
	out, err := cmd.Output()
	result := string(out)
	// get error string from certificate generation script
	err = utils.WrapError(err)
	logger.V(3).Info("Cert generation", "out", result, "err", err)
	if result != "" {
		updated = true
		output = readOutput(logger, result)
	}
	logger.V(3).Info("Returning", "err", err, "updated", updated)
	return err, updated, output
}

func readOutput(logger logr.Logger, raw string) []interface{} {
	genLogs := []map[string]string{}
	err := yaml.Unmarshal([]byte(raw), &genLogs)
	if err != nil {
		logger.Error(err, "Unable to unmarshal cert_generation to structured output")
		logger.Info("cert_generation output", "output", raw)
		return nil
	}

	result := []interface{}{}
	for _, eventlog := range genLogs {
		keyvalues := []interface{}{}
		for k, v := range eventlog {
			keyvalues = append(keyvalues, k, v)
		}
		result = append(result, keyvalues)
		logger.Info("cert_generation output", keyvalues...)
	}

	return result
}
